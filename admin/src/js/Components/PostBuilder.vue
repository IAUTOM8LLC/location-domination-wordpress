<template>
    <div
        v-if="
            (!previousRequestTransient || request.cancelled) && !request.started
        "
    >
        ...
        <template v-if="gridForm.country && gridForm.country.id === 236">
            <select-input
                name="ld[group]"
                v-model="gridForm.group"
                label="How would you like to build posts?"
            >
            ...
            <input
                v-if="
                    gridForm.group &&
                    gridForm.group !== 'For all cities/counties'
                "
                type="hidden"
                name="ld[states]"
                :value="statesJoined"
            ...
            <input
                v-if="
                    gridForm.group &&
                    (gridForm.group === 'For specific counties' ||
                        gridForm.group === 'For specific cities')
                "
                type="hidden"
                name="ld[counties]"
            ...
            <input
                v-if="
                    gridForm.group && gridForm.group === 'For specific cities'
                "
                type="hidden"
                name="ld[cities]"
                :value="citiesJoined"
            ...
            <advanced-select-input
                ...
                :options="states"
                label-key="state"
                label="Select states to target"
            />
            <advanced-select-input
                :preselect="preselect.counties"
                @input="updateCities"
                name="counties[]"
                v-model="gridForm.counties"
                v-if="
                    gridForm.group &&
                    (gridForm.group === 'For specific counties' ||
                        gridForm.group === 'For specific cities')
        ...
        <template v-else>
            ...
            <advanced-select-input
                ...
                v-if="
                    gridForm.group &&
                    gridForm.group !== 'For all cities/regions'
                "
                :close-on-select="false"
                :multiple="true"
                track-by="id"
                :options="regions"
                label-key="name"
                label="Select regions to target"
            />
            <advanced-select-input
                :preselect="preselect.cities"
                @input="updateSuburbs"
            ...
            <advanced-select-input
                ...
                :options="suburbs"
                label-key="suburb"
                label="Select suburbs to target"
            />
        </template>

        <button
            type="button"
            @click.prevent="buildPosts"
            style="background: #3356ca"
        ...
        <div class="relative py-3">
            <div class="flex mb-2 items-center justify-between">
                <div>
                    <span
                        :class="{
                            'text-red-600 bg-red-200': !request.completed,
                            'text-green-800 bg-green-400': request.completed,
                        }"
                ...
                <div class="text-right">
                    <span
                        :class="{
                            'text-red-600': !request.completed,
                            'text-green-800': request.completed,
                        }"
            ...
            <div
                :class="{
                    'bg-red-200': !request.completed,
                    'bg-green-400': request.completed,
                }"
                class="overflow-hidden h-2 mb-4 text-xs flex rounded"
            >
                <div
                    :class="{
                        'bg-red-600': !request.completed,
                        'bg-green-500': request.completed,
                    }"
                    :style="{ width: request.progress + '%' }"
                    class="shadow-none progress flex flex-col text-center whitespace-nowrap text-white justify-center"
                ></div>
            </div>
        </div>
    </div>
    ...
    <div v-else>
        <p class="text-gray-600 text-base text-center mt-5 mb-4">
            You already have a post request mid-build. Would you like to
            continue or cancel?
        </p>

        <div class="mt-8 flex justify-center mb-6">
            <button
                @click.prevent="continuePosts"
                type="button"
                class="bg-blue-700 px-5 py-2 mr-4 text-white font-semibold"
            >
                Continue
            </button>
            <button
                @click.prevent="cancelPosts"
                type="button"
                class="bg-transparent self-center block text-blue-500 font-semibold"
            >
                Cancel
            </button>
        </div>
...
export default {
    ...
    computed: {
        ...
        stateIds() {
            return this.gridForm.hasOwnProperty('states') &&
                this.gridForm.states
                ? this.gridForm.states.map((item) => {
                      return item.id;
                  })
                : [];
        },
        ...
        regionIds() {
            return this.gridForm.hasOwnProperty('regions') &&
                this.gridForm.regions
                ? this.gridForm.regions.map((item) => {
                      return item.id;
                  })
                : [];
        },
        ...
        cityIds() {
            return this.gridForm.hasOwnProperty('cities') &&
                this.gridForm.cities
                ? this.gridForm.cities.map((item) => {
                      return item.id;
                  })
                : [];
        },
        ...
        countiesIds() {
            return this.gridForm.hasOwnProperty('counties') &&
                this.gridForm.counties
                ? this.gridForm.counties.map((item) => {
                      return item.id;
                  })
                : [];
        },
        ...
        suburbIds() {
            return this.gridForm.hasOwnProperty('suburbs') &&
                this.gridForm.suburbs
                ? this.gridForm.suburbs.map((item) => {
                      return item.id;
                  })
                : [];
        },
    ...
    data() {
        return {
            groupOptions: [
                'For all cities/counties',
                'For specific states',
                'For specific counties',
                'For specific cities',
            ],
            otherCountryOptions: [
                'For all cities/regions',
    ...
    beforeMount() {
        if (this.previousRequest) {
            if (
                ...
                this.previousRequest.hasOwnProperty('group')
            ) {
                this.gridForm = Object.assign({}, this.gridForm, {
                    group: this.previousRequest.group,
                });
            }
    ...
    mounted() {
        const _this = this;
        const populationcheckbox = document.getElementById(
            'acf-field_8ff41b8044an1212'
        );
        const populationselect = document.querySelectorAll(
            '.acf-field-8ff41b8044an1212d'
        );
        ...
        ExternalRepository.getCountries().then((Response) => {
            ...
            if (
                this.previousRequest &&
                this.previousRequest.hasOwnProperty('country')
            ) {
        ...
        ExternalRepository.getStates().then((Response) => {
            ...
            if (
                this.previousRequest &&
                this.previousRequest.hasOwnProperty('states')
            ) {
                this.preselect.states = this.previousRequest.states
                    .map((id) => {
        ...
        axios
            .get(
                `${this.ajaxUrl}?action=location_domination_get_settings&apiKey=1`
            )
            .then(({ data }) => {
        ...
        axios
            .get(
                `${this.ajaxUrl}?action=location_domination_get_settings&uuid=1&post=${this.templateId}`
            )
            .then(({ data }) => {
    ...
    methods: {
        ...
        cancelPosts() {
            return ExternalRepository.cancelLocalQueuePostRequest(
                this.ajaxUrl,
                this.templateId,
                this.nonce
            ).then(() => {
        ...
        buildPosts() {
            ...
            ExternalRepository.previewPostRequest(
                form,
                url,
                this.templateId,
                this.nonce
        ...
        continuePosts() {
            ...
            ExternalRepository.continueLocalQueuePostRequest(
                url,
                this.templateId,
                this.nonce
            ).then(({ data }) => {
                    ...
                    if (parseInt(data.batches_needed) > 1) {
                        ...
                        const interval = setInterval(() => {
                            ...
                            _this.pollWorker().then(({ data }) => {
                                ...
                                const estimated_time_remaining = parseFloat(
                                    data.estimated_time_remaining
                                );
        ...
        updateWorldCities() {
            ...
            if (
                (!this.gridForm.country || this.gridForm.country.id === 236) &&
                _this.stateIds.length === 0
            ) {
                // _this.counties = [];
            } else {
                this.debounces.cities = setTimeout(() => {
                    ExternalRepository.getWorldCities({
                        ...
                        if (
                            this.previousRequest &&
                            this.previousRequest.hasOwnProperty('cities')
                        ) {
                            this.preselect.cities = this.previousRequest.cities
                                .map((id) => {
                                    const match = _cities.filter(function (
                                        city
                                    ) {
            ...
            if (
                ...
                this.debounces.suburbs = setTimeout(() => {
                    ExternalRepository.getSuburbs({
                        ...
                        if (
                            this.previousRequest &&
                            this.previousRequest.hasOwnProperty('suburbs')
                        ) {
                            this.preselect.suburbs =
                                this.previousRequest.suburbs
                                    .map((id) => {
                                        const match = _suburbs.filter(function (
                                            suburb
                                        ) {
            ...
            if (_this.gridForm.counties.length > 0) {
                this.debounces.counties = setTimeout(() => {
                    ExternalRepository.getCities({
                        ...
                        this.$nextTick(() => {
                            if (
                                _this.previousRequest.hasOwnProperty(
                                    'cities'
                                ) &&
                                _this.preselect.cities === false
                            ) {
                                _this.preselect.cities =
                                    _this.previousRequest.cities
                                        .map((id) => {
                                            const match = _cities.filter(
                                                function (city) {
                                                    return (
                                                        parseInt(id) === city.id
                                                    );
        ...
        updateRegions() {
            ...
            this.debounces.regions = setTimeout(() => {
                ExternalRepository.getRegions({
                    ...
                    if (
                        this.previousRequest &&
                        this.previousRequest.hasOwnProperty('regions')
                    ) {
                        this.preselect.regions = this.previousRequest.regions
                            .map((id) => {
                                const match = _regions.filter(function (
                                    region
                                ) {
                    ...
                    ExternalRepository.getCounties({
                        ...
                        this.$nextTick(() => {
                            ...
                            if (
                                ...
                                _this.preselect.counties === false
                            ) {
                                _this.preselect.counties =
                                    _this.previousRequest.counties
                                        .map((id) => {
                                            const match = _counties.filter(
                                                function (county) {
                                                    return (
                                                        parseInt(id) ===
                                                        county.id
